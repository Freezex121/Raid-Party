...
{
    return (Rectangle){ (float)((VIRT_W / 2) - 120), 98.0f, 110.0f, (float)BTN_H };
}

static Rectangle rest_upgrade_button(void)
{
    return (Rectangle){ (float)((VIRT_W / 2) + 10), 98.0f, 110.0f, (float)BTN_H };
}
...
void rest_screen_draw(void)
{
    ...
    if (mode == REST_CHOICE)
    {
        DrawText("REST SITE", (VIRT_W / 2) - MeasureText("REST SITE", 14) / 2, 24, 14, RAYWHITE);
        DrawText("Choose:", (VIRT_W / 2) - MeasureText("Choose:", 7) / 2, 48, 7, (Color){ 160, 160, 180, 200 });
        ...
        DrawText("HEAL PARTY", (int)(heal_btn.x + heal_btn.width / 2 - MeasureText("HEAL PARTY", 6) / 2), (int)heal_btn.y + 5, 6, RAYWHITE);

        DrawRectangleRec(upg_btn, uc);
        DrawText("UPGRADE A CARD", (int)(upg_btn.x + upg_btn.width / 2 - MeasureText("UPGRADE A CARD", 6) / 2), (int)upg_btn.y + 5, 6, RAYWHITE);

        DrawText("Fully restore HP", (int)heal_btn.x + 7, (int)heal_btn.y + 22, 5, (Color){ 160, 220, 170, 200 });
        DrawText("Boost one card's power", (int)upg_btn.x + 4, (int)upg_btn.y + 22, 5, (Color){ 160, 180, 220, 200 });

        if (rest_msg[0])
            DrawText(rest_msg, (VIRT_W / 2) - MeasureText(rest_msg, 6) / 2, 132, 6, (Color){ 200, 150, 100, 220 });
    }
    else if (mode == REST_UPGRADE)
    {
        ...
        if (hovered_upgrade >= 0)
        {
            ...
            {
                DrawText("Already upgraded", 10, VIRT_H - 18, 5, (Color){ 160, 160, 180, 220 });
            }
            else
            {
                ...
                int x = 10;
                if (od > 0) { char b[32]; snprintf(b, sizeof(b), "DMG %d>%d", od, nd); DrawText(b, x, VIRT_H - 18, 5, (Color){ 200, 100, 100, 255 }); x += MeasureText(b, 5) + 8; }
                if (oh > 0) { char b[32]; snprintf(b, sizeof(b), "HEAL %d>%d", oh, nh); DrawText(b, x, VIRT_H - 18, 5, (Color){ 100, 200, 100, 255 }); x += MeasureText(b, 5) + 8; }
                if (os > 0) { char b[32]; snprintf(b, sizeof(b), "SHIELD %d>%d", os, ns); DrawText(b, x, VIRT_H - 18, 5, (Color){ 100, 180, 255, 255 }); }
            }
        }
    }
    else if (mode == REST_HEAL)
    {
        DrawText("PARTY RESTED", (VIRT_W / 2) - MeasureText("PARTY RESTED", 12) / 2, 100, 12, (Color){ 100, 220, 120, 255 });
